...
c
     ...
     &			 fi(0:8,nxd,nyd)
	     ...
	     enddo
c------------------------------------------------------------------
c key=0:  start from the IC's specified by this code.
c	   		i.e. potential flow for flow over cylinder.
c key=1:  start from the saved field. 
c	  No immediate nested iteration 
c key=2:  perform nested iteration immediately; after reading 
c	  the saved field.
	     ...
	     write(25,*)'variables ="n", "cdx-mrt", "cdy-mrt"'
	     write(25,*)'zone T="drag_mrt"'
...
c  checking convergence
         ...
         write(*,885) ii,err2,i_max,j_max,err_max
...
c  output-------------------------------------------------------
         ...
         if(kw.eq.1) then
	       i=imid
	       write(12,*)'ii=',ii,'  err2=',err2
c printing some typical velocity and derivative profiles:
c note: the i=imid line goes through the center of the cylinder
	       do i=2,nx-1
	          sum_flux(i)=0
	          do j=2,ny-2
...
c ----------------------------------------------------------------
c  save the flow field every 1000 time steps:
c  note: results for ux,uy, rho at i=1, i=nx, j=1 and j=ny lines are 
c	 not physical.  Only some of the fi's are needed at those
c	 locations; 
	     ...
	     if(kw.eq.1) then
           ...
           write(20,*) 'zone I=', ny,'J=',nx,'f= point'   !!!!!!
	       do i=1,nx
	         do j=1,ny
...
c  set boundary node velocity at (i,j) where wb(i,j)=1
		  ratio=(del-1)/del
...
c  evaluate the equilibrium function on the solid nodes: wb(i,j)=1
...
c  obtain the inlet conditions for fi's at i=1
...
c  set boundary node velocity at i=1
		 vxx=vx(ia,ja)
		 vyy=vy(ia,ja)
...
c  carry out relaxation step for the interior points
...
500	   continue	
	enddo
c  simple extrapolation at the downstrem.
...
c wb(i,j)= 1 for bounadary nodes and 0 for the rest of points.

c The force (fx,fy) actually is the force of the solid on the fluid.
...
100	   continue
	...
	enddo
648	format(3x,3i4,3x,f12.6)
688	format(3x,i4,3x,f12.6)
	write(12,*)'k_del_max=',k_del_max
c	write(6,*)'done finding_dels'
	...
	forall(i=1:nx,j=1:ny,walls(i,j).eq.1.and.
     ...
     &     	+walls(i,j-1)+walls(i+1,j-1)-8).lt.0) wb(i,j)=1
	   ...
	   do i=1,nx
	      ...
	      do j=1,ny
		 yy=j-jmid
c		 if(wb(i,j).eq.1.0) write(24,*) xx, yy
		 if(wb(i,j).eq.1.0) write(24,*) i,j
	      enddo
	    enddo
c	write(6,*)'finishing wall boundary identification'
	return
	end
c**************************************************************************
...
255	     format(2x,4f12.7)
	     ...
	     swh=99
	     sw4=99
	     sw_diag=walls(im,jp)+walls(ip,jm)+walls(ip,jp)+walls(im,jm)
...
c initialize physical field for flow over a cylinder: rho, ux, uy
c using potential flow field
...
c 	 
	   ...
	   ux(1,:)=vt
	   uy(1,:)=0.0
	   write(6,*)'finishing specifying ux,uy from potential sol'
	   ...
	   do i=1,nx
	      do j=1,ny
	         un(i,j)=ux(i,j)
              enddo
           enddo
	   write(6,*)'finishing 1st macro for key=0'
	endif
...
c	   write(6,*)'done initialization for key=2'
      	   call macro_variables
c	   write(6,*)'done  macro for key=2'
	endif
	...
	write(12,*)'finishing initialization; here is the ux(imid,y)'
     ...
     ^			    uy(imid,j),  rho(imid,j)
	       write(15,406) j,float(j-jmid)/radius,ux(i_front,j)/vt,
     ^			     uy(i_front,j), rho(i_front,j)
	   enddo
...
c	write(6,*)'done initialization for key=3'

      	call macro_variables
c	write(6,*)'done  macro for key=3'
